...
실행 환경에 따라서 테스트가 실패할 수 있다. 램과 CPU 코어 수가 영향을 미치기도 한다. 만약 테스트 실패의 원인이 명확하지 않다면 `jest.config.ts`에서 `testTimeout` 값을 60초 이상으로 설정하고, `maxWorkers` 값을 2로 설정해 본다.

> 32GB, 8cores 환경에서 단독으로 실행하면 5초에 완료되는 테스트가 다른 테스트와 함께 실행하면 10초를 초과해서 timeout이 발생하는 경우도 있었다.
