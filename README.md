...
마이크로서비스 아키텍처를 사용하는 영화 예매 웹사이트를 구현한 참조용 NestJS 애플리케이션으로, 다음과 같은 특징을 가집니다:
...
원하는 프로젝트 이름으로 변경하려면 다음 설정을 검토하고 수정하세요:
...
이 프로젝트에서 사용하는 인프라를 변경하려면 다음 설정을 검토하고 수정하세요:
...
이 프로젝트의 devcontainer는 아래와 같이 기본 한국어 환경을 설정해 두었습니다. 영어 이외의 언어를 사용하려면, 각 언어에 맞게 설정을 수정하세요.
...
이 프로젝트는 테스트 시 MongoDB, Redis, NATS 등을 사용합니다. 이를 위해 메모리 기반 DB 라이브러리(mongodb-memory-server 등)를 활용하는 방법도 있지만, 여기서는 실제 운영 환경과 유사하게 도커 컨테이너로 구성하여 테스트를 진행합니다. 이렇게 하면 다음과 같은 장점이 있습니다:

- **운영 환경과 유사함**: 실제 배포 환경에서처럼 MongoDB, Redis, NATS를 컨테이너로 띄우므로, 클러스터나 네트워크 문제 등 실제 인프라에서 발생할 수 있는 오류를 사전에 발견하기 쉽습니다.
- **간편한 설정**: 테스트를 위해 별도의 라이브러리를 추가로 학습하거나 설정할 필요가 없으므로, 개발 환경을 간결하게 유지할 수 있습니다.

이 방식의 단점은 MongoDB가 메모리를 많이 사용하고 그로 인해 테스트가 불규칙하게 실패할 수 있다는 것입니다. 그래서 전체 테스트를 실행하면 인프라를 초기화 하도록 구성했습니다. 자세한 사항은 `scripts/run-test.sh`를 참고하세요.
...
실행 환경에 따라서 테스트가 실패할 수 있습니다. 램과 CPU 코어 수가 영향을 미치기도 합니다. 만약 테스트 실패의 원인이 명확하지 않다면 jest.config.ts에서 testTimeout 값을 60초 이상으로 설정하고, maxWorkers 값을 2로 조정해 보세요.

> 예를 들어, 32GB/8코어 환경에서 단독으로 실행할 때는 5초 만에 끝나는 테스트도, 다른 테스트와 병렬로 실행하면 10초를 넘어 타임아웃이 발생하는 사례가 있었습니다.

## 7. 서비스 실행 및 디버깅

이 프로젝트는 마이크로서비스 구조 특성상, 각 서비스를 따로 실행하기보다 통합 테스트를 활용해 검증하는 방식을 권장합니다. 그럼에도 불구하고 특정 서비스를 단독으로 실행해 디버깅해야 한다면, 아래 내용을 참고하세요.

- `/.vscode/launch.json`

## 8. 빌드 및 E2E 테스트

`npm run test:e2e`을 실행하면 프로젝트 빌드부터 실행까지 전체 과정을 테스트합니다. 빌드 및 실행에 필요한 자세한 사항은 이 명령에 사용된 스크립트를 참고하세요.
...
이 프로젝트의 설계 문서는 PlantUML을 사용해서 작성했습니다. 설계 문서는 `./docs/designs`에 있으며 이것을 보려면 PlantUML(jebbs.plantuml) 확장을 설치해야 합니다.
